    ...
    <div className="space-y-6 p-6">
      ...
      <div className="gap-6 grid grid-cols-1 md:grid-cols-7">
        ...
        <Card className="border-l-4 border-l-green-500">
          ...
          <CardContent>
            <div className="font-bold text-green-600 text-2xl">{accountSummaries.length}</div>
            <p className="text-muted-foreground text-xs">
              Active accounts
            </p>
          </CardContent>
                ...
                <div key={job.id} className="flex justify-between items-center hover:bg-gray-50 p-3 border rounded-lg cursor-pointer" onClick={() => handleJobClick(job.id)}>
                  ...
                  <div className="text-right">
                    ...
                    <Badge className={`text-xs ${getStatusColor(job.job_status || job.status)}`}>
                      {job.job_status || job.status || 'Open'}
                    </Badge>
                  </div>
                </div>
              ))}
          ...
          <div className="gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {accountSummaries.map((account) => (
              <div 
                ...
                className="hover:bg-gray-50 hover:shadow-md p-4 border rounded-lg transition-all cursor-pointer"
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  ...
                  <Badge variant="outline" className="text-xs">
                    {account.open_jobs} open
                  </Badge>
                </div>
                
                <h3 className="mb-1 font-semibold text-sm">{account.company_name}</h3>
                <p className="mb-2 text-gray-500 text-xs">#{account.account_number}</p>
                ...
                <div className="space-y-1 text-gray-600 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatCurrency(account.total_value)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Last Activity:</span>
                    <span className="font-medium">{formatTimeAgo(account.last_activity)}</span>
                  </div>
                </div>
                
                <div className="flex justify-between items-center mt-3">
                  <span className="font-medium text-blue-600 text-xs">Click to view details</span>
