...
interface FCAnnuityInvoicesSectionProps {
  costCodes: string;
}

export default function FCAnnuityInvoicesSection({ costCodes }: FCAnnuityInvoicesSectionProps) {
  ...
  const fetchInvoices = async (search = "", month = selectedMonth) => {
    try {
      ...
      setInvoices(allInvoices.filter((inv) => inv.source_type === "account_invoice"));
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-bold text-gray-900 text-xl">Annuity Invoices</h1>
          <p className="text-xs text-gray-500">Annuity billing invoices for this account</p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={10} className="text-center text-xs text-slate-400 py-8">
                    No annuity invoices found{selectedMonth ? ` for ${selectedMonth}` : ""}
                  </TableCell>
                </TableRow>
              ) : (
