...
interface FCJobCardInvoicesSectionProps {
  costCodes: string;
}

export default function FCJobCardInvoicesSection({ costCodes }: FCJobCardInvoicesSectionProps) {
  const [invoices, setInvoices] = useState<JobCardInvoiceRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const currentMonth = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}`;
  const [selectedMonth, setSelectedMonth] = useState(currentMonth);
  const [selectedInvoice, setSelectedInvoice] = useState<JobCardInvoiceRow | null>(null);
  ...
  const fetchInvoices = async (search = "", month = selectedMonth) => {
    try {
      ...
      const allInvoices = Array.isArray(result?.invoices) ? result.invoices : [];
      setInvoices(allInvoices.filter((inv) => inv.source_type === "job_card_invoice"));
    } catch (error) {
  ...
  const downloadInvoiceExcel = async (invoice: JobCardInvoiceRow) => {
  ...
  const handleOpenInvoice = async (invoice: JobCardInvoiceRow) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-bold text-gray-900 text-xl">Job Card Invoices</h1>
          <p className="text-xs text-gray-500">Invoices generated from job cards</p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={10} className="text-center text-xs text-slate-400 py-8">
                    No job card invoices found{selectedMonth ? ` for ${selectedMonth}` : ""}
                  </TableCell>
                </TableRow>
              ) : (
