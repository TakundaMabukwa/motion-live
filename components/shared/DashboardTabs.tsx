      ...
      <TabsList className="flex h-auto w-full flex-wrap justify-start gap-1 border bg-white p-1">
        {tabs.map((item) => (
          <TabsTrigger 
            key={item.value}
            value={item.value} 
            className="flex shrink-0 items-center space-x-2 whitespace-nowrap data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600"
          >
            {item.icon && <item.icon className="w-4 h-4" />}
            <span className={tabs.length > 6 ? 'text-xs' : ''}>{item.label}</span>
          </TabsTrigger>
        ))}
      </TabsList>
      {tabs.map((item) => (
