...
export async function POST(request: NextRequest) {
  try {
    ...
    const { payments, paymentReference, paymentMethod, notes } =
      (requestBody as {
        payments?: unknown;
    ...
    for (const [index, payment] of payments.entries()) {
      try {
        ...
        const normalizedPaymentPeriodType =
          String(payment.paymentPeriodType || "").trim().toLowerCase() === "outstanding"
            ? "outstanding"
            : "current";
        ...
        if (payment.accountNumber && normalizedPaymentPeriodType === "outstanding") {
          ...
          if (agingError) {
            errors.push(
              `${normalizedAccountNumber}: ${agingError.message || "Failed to load outstanding age analysis"}`,
            );
            continue;
          }
        ...
        if (!invoice && payment.accountNumber) {
          ...
          if (costCenterError) {
            errors.push(
              `${normalizedAccountNumber}: ${costCenterError.message || "Failed to prepare payment invoice"}`,
            );
            continue;
          }
          ...
          if (
            ...
            try {
              const allocation = await allocateTrackedInvoiceNumber(supabase, {
                source: "api/payments/bulk-process:outstanding-auto-invoice",
                userId: user?.id || null,
              ...
              errors.push(
                `${normalizedAccountNumber}: Failed to allocate invoice number`,
              );
              continue;
            }
            ...
            const { data: insertedInvoice, error: insertError } = await supabase
              ...
              .insert({
                ...
                notes: "Auto-created during bulk payment from outstanding age analysis snapshot",
            ...
            if (insertError || !insertedInvoice) {
              await markTrackedInvoiceFailed(supabase, {
                ...
                errorMessage:
                  insertError?.message ||
                  "Failed to create outstanding invoice snapshot",
              });
              errors.push(
                `${normalizedAccountNumber}: ${insertError?.message || "Failed to create outstanding invoice snapshot"}`,
              );
              continue;
            }
            ...
            try {
              const allocation = await allocateTrackedInvoiceNumber(supabase, {
                source: "api/payments/bulk-process:current-auto-invoice",
                userId: user?.id || null,
        ...
        if (normalizedPaymentPeriodType === "outstanding" && outstandingAgingRow) {
          ...
          if (paymentsMirrorUpdateError) {
            errors.push(`${invoice.account_number}: ${paymentsMirrorUpdateError.message}`);
            continue;
          }
        } else {
