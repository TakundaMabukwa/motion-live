...
const isSingleTechnicianEmail = (value: unknown) => {
...
export async function GET() {
  try {
    ...
    const stockRows = (Array.isArray(technicianStockRows) ? technicianStockRows : []).filter(
      (row) =>
        isSingleTechnicianEmail(row?.technician_email) &&
        normalizeEmail(row?.technician_email) === technicianEmail,
    );
    const technicianEmailFromRows =
      stockRows.find((row) => normalizeEmail(row?.technician_email))?.technician_email ||
      technicianEmail;
    ...
    processedStock.forEach((item) => {
      const dedupeKey = [
        normalizeEmail(item.code),
        normalizeEmail(item.description),
        normalizeEmail(item.supplier),
        normalizeEmail(item.serial_number || item.ip_address || ''),
      ].join('|');

      const currentQty = parseInt(String(item.quantity || '0')) || 0;
      const existing = dedupedStockMap.get(dedupeKey);
      if (!existing) {
        dedupedStockMap.set(dedupeKey, { ...item, quantity: String(currentQty) });
        return;
      }
      const existingQty = parseInt(String(existing.quantity || '0')) || 0;
      existing.quantity = String(existingQty + currentQty);
      dedupedStockMap.set(dedupeKey, existing);
    });

    const finalStock = Array.from(dedupedStockMap.values());
    console.log(`Processed ${finalStock.length} assigned parts for technician ${technicianEmailFromRows}`);

    return NextResponse.json({ 
      stock: finalStock,
      total_items: finalStock.length
    });
  } catch (error) {
