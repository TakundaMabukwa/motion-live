...
interface PricingItem {
  ...
  forex_base_price?: number | string;
  current_roe?: number | string;
  gl_group?: string;
...
function StockOrdersContent() {
  ...
  const [editingPricingData, setEditingPricingData] =
    useState<Partial<PricingItem> | null>(null);
  const [savingPricingId, setSavingPricingId] = useState<number | null>(null);
  const [bulkEditMode, setBulkEditMode] = useState(false);
  const [bulkEditingMap, setBulkEditingMap] = useState<
    Record<number, Partial<PricingItem>>
  >({});
  ...
  const fetchStockPricing = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("stock_pricing")
        .select(
          "id, description, cost_excl_vat_zar, USD, supplier, stock_type, preferred_supplier, supplier_product_code, purchasing_currency, forex_base_price, current_roe, gl_group, requires_serial_number, is_tangible, valuation_method, override_tax_type, created_at",
        );
  ...
  const startEditPricing = (item: PricingItem) => {
    ...
    setEditingPricingData({
      ...
      purchasing_currency: item.purchasing_currency,
      forex_base_price: item.forex_base_price,
      current_roe: item.current_roe,
  ...
  const startBulkEdit = () => {
    ...
    pricingItems.forEach((p) => {
      map[p.id] = {
        ...
        purchasing_currency: p.purchasing_currency,
        forex_base_price: p.forex_base_price,
        current_roe: p.current_roe,
        gl_group: p.gl_group,
        requires_serial_number: p.requires_serial_number,
        is_tangible: p.is_tangible,
        valuation_method: p.valuation_method,
        override_tax_type: p.override_tax_type,
      };
    });
  ...
  const saveBulkEdit = async () => {
    try {
      ...
      const updates = Object.keys(bulkEditingMap).map(async (key) => {
        ...
        if (data.cost_excl_vat_zar !== undefined)
          payload.cost_excl_vat_zar = Number(data.cost_excl_vat_zar || 0);
        if (data.USD !== undefined)
          payload.USD = data.USD === "" ? null : Number(data.USD);
        if (data.stock_type !== undefined) payload.stock_type = data.stock_type;
        if (data.preferred_supplier !== undefined)
          payload.preferred_supplier = data.preferred_supplier;
        if (data.supplier_product_code !== undefined)
          payload.supplier_product_code = data.supplier_product_code;
        if (data.purchasing_currency !== undefined)
          payload.purchasing_currency = data.purchasing_currency;
        if (data.forex_base_price !== undefined)
          payload.forex_base_price =
            data.forex_base_price === "" ? null : Number(data.forex_base_price);
        if (data.current_roe !== undefined)
          payload.current_roe =
            data.current_roe === "" ? null : Number(data.current_roe);
        if (data.gl_group !== undefined) payload.gl_group = data.gl_group;
        if (data.requires_serial_number !== undefined)
          payload.requires_serial_number = Boolean(data.requires_serial_number);
        if (data.is_tangible !== undefined)
          payload.is_tangible = Boolean(data.is_tangible);
        if (data.valuation_method !== undefined)
          payload.valuation_method = data.valuation_method;
        if (data.override_tax_type !== undefined)
          payload.override_tax_type = data.override_tax_type;
      ...
      setPricingItems((prev) => {
        ...
        results.forEach((r) => {
          ...
          if (r.updated) {
            copy[idx] = {
              ...copy[idx],
              ...(r.updated as Partial<PricingItem>),
            };
          } else {
            // fallback: apply from bulk map
            copy[idx] = {
              ...copy[idx],
              ...(bulkEditingMap[r.id] as Partial<PricingItem>),
            } as PricingItem;
          }
        });
        return copy;
      ...
      toast({
        title: "Saved",
        description: "All pricing items updated",
        variant: "default",
      });
      setBulkEditingMap({});
      setBulkEditMode(false);
      ...
      toast({
        title: "Error",
        description: `Failed to save some items: ${msg}`,
        variant: "destructive",
      });
    } finally {
      setSavingAll(false);
  ...
  const saveEditPricing = async (id: number) => {
    ...
    try {
      ...
      if (editingPricingData.description !== undefined)
        updatePayload.description = editingPricingData.description;
      if (editingPricingData.supplier !== undefined)
        updatePayload.supplier = editingPricingData.supplier;
      if (editingPricingData.cost_excl_vat_zar !== undefined)
        updatePayload.cost_excl_vat_zar = Number(
          editingPricingData.cost_excl_vat_zar || 0,
        );
      if (editingPricingData.USD !== undefined)
        updatePayload.USD =
          editingPricingData.USD === "" ? null : Number(editingPricingData.USD);
      if (editingPricingData.stock_type !== undefined)
        updatePayload.stock_type = editingPricingData.stock_type;
      if (editingPricingData.preferred_supplier !== undefined)
        updatePayload.preferred_supplier =
          editingPricingData.preferred_supplier;
      if (editingPricingData.supplier_product_code !== undefined)
        updatePayload.supplier_product_code =
          editingPricingData.supplier_product_code;
      if (editingPricingData.purchasing_currency !== undefined)
        updatePayload.purchasing_currency =
          editingPricingData.purchasing_currency;
      if (editingPricingData.forex_base_price !== undefined)
        updatePayload.forex_base_price =
          editingPricingData.forex_base_price === ""
            ? null
            : Number(editingPricingData.forex_base_price);
      if (editingPricingData.current_roe !== undefined)
        updatePayload.current_roe =
          editingPricingData.current_roe === ""
            ? null
            : Number(editingPricingData.current_roe);
      if (editingPricingData.gl_group !== undefined)
        updatePayload.gl_group = editingPricingData.gl_group;
      if (editingPricingData.requires_serial_number !== undefined)
        updatePayload.requires_serial_number = Boolean(
          editingPricingData.requires_serial_number,
        );
      if (editingPricingData.is_tangible !== undefined)
        updatePayload.is_tangible = Boolean(editingPricingData.is_tangible);
      if (editingPricingData.valuation_method !== undefined)
        updatePayload.valuation_method = editingPricingData.valuation_method;
      if (editingPricingData.override_tax_type !== undefined)
        updatePayload.override_tax_type = editingPricingData.override_tax_type;
      ...
      if (data && data[0]) {
        setPricingItems((prev) =>
          prev.map((p) =>
            p.id === id ? { ...p, ...(data[0] as Partial<PricingItem>) } : p,
          ),
        );
      } else {
        setPricingItems((prev) =>
          prev.map((p) =>
            p.id === id ? ({ ...p, ...editingPricingData } as PricingItem) : p,
          ),
        );
      }
      ...
      toast({
        title: "Saved",
        description: "Pricing item updated",
        variant: "default",
      });
      cancelEditPricing();
    } catch (err: unknown) {
  ...
  const filteredApprovedOrders = filteredOrders.filter(
    (order) => order.approved,
  );

  const totalAmount = orders.reduce((sum, order) => sum + order.totalAmount, 0);
  const approvedAmount = approvedOrders.reduce(
          ...
          <nav className="mt-6">
            <div className="space-y-2 px-4">
              <Link
                ...
                className="flex items-center hover:bg-gray-50 px-3 py-2 rounded-md text-gray-600 hover:text-gray-900"
              >
              ...
              <Link
                href="/protected/master/invoices"
                className="flex items-center hover:bg-gray-50 px-3 py-2 rounded-md text-gray-600 hover:text-gray-900"
              >
              ...
              <Link
                href="/protected/master/stock-orders"
                className="flex items-center bg-blue-50 px-3 py-2 rounded-md text-blue-600"
              >
                Stock Orders
              </Link>
                          ...
                          {pricingItems.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                              <td className="px-6 py-4 whitespace-nowrap">
                                {(bulkEditMode && bulkEditingMap[item.id]) ||
                                editingPricingId === item.id ? (
                                  <input
                                    value={
                                      bulkEditMode
                                        ? (bulkEditingMap[item.id]
                                            ?.description ?? "")
                                        : (editingPricingData?.description ??
                                          "")
                                    }
                                    onChange={(e) => {
                                      if (bulkEditMode)
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                editingPricingId === item.id ? (
                                  <input
                                    value={
                                      bulkEditMode
                                        ? (bulkEditingMap[item.id]?.supplier ??
                                          "")
                                        : (editingPricingData?.supplier ?? "")
                                    }
                                    onChange={(e) => {
                                      if (bulkEditMode)
                                        setBulkEditingMap((prev) => ({
                                  ...
                                  <input
                                    value={String(
                                      bulkEditMode
                                        ? (bulkEditingMap[item.id]
                                            ?.cost_excl_vat_zar ?? "")
                                        : (editingPricingData?.cost_excl_vat_zar ??
                                            ""),
                                    )}
                                    onChange={(e) => {
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                {editingPricingId === item.id ? (
                                  <div className="flex items-center space-x-2">
                                    <button
                                      onClick={() => saveEditPricing(item.id)}
                                      disabled={savingPricingId === item.id}
                                    ...
                                    <button
                                      ...
                                      className="bg-gray-100 px-3 py-1 rounded text-xs"
                                    >
                                      Cancel
                                    </button>
                                  </div>
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <button
                                      ...
                                      className="bg-white border px-3 py-1 rounded text-xs"
                                    >
                                      Edit
                                    </button>
                                  </div>
              ...
              <div className="flex items-center space-x-2">
                {selectedOrder?.invoiceLink && (
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      selectedOrder && handleDownloadInvoice(selectedOrder)
                    }
                  >
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div>
                        ...
                        {getStatusBadge(selectedOrder.approved)}
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        <span className="font-medium">Total Amount (ZAR):</span>{" "}
                      ...
                      {selectedOrder.totalAmountUSD && (
                        <div>
                          ...
                          ${selectedOrder.totalAmountUSD.toLocaleString()}
                        </div>
                      )}
                      <div>
                      ...
                      <div className="bg-gray-50 rounded-lg overflow-hidden">
                        <table className="divide-y divide-gray-200 min-w-full">
                          <thead className="bg-gray-100">
                            <tr>
                              <th className="px-4 py-2 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">
                                Item
                              </th>
                              <th className="px-4 py-2 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">
                                Quantity
