...
const mockVehicles = [
  {
    id: 1,
    registration: 'LX 90 MH GP',
    make: 'SUZUKI',
    model: '1.2 P/V',
    costCentre: 'AIRCASS COMPRESSORS (PTY) LTD',
    fleetNumber: 'DCS AFRICA',
    productCount: 4,
    color: 'White',
    year: 2022,
    fuelType: 'Petrol'
...
// Steps for the create vehicle form
const steps = [
...
function CreateVehicle({ onBack, onSave }: { onBack: () => void; onSave: () => void }) {
          ...
          <div className="flex justify-between items-center">
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full ${currentStep >= step.id ? 'bg-sky-500 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  {currentStep > step.id ? (
                ...
                <span className={`ml-2 text-sm font-medium ${currentStep >= step.id ? 'text-sky-600' : 'text-gray-600'
                  }`}>
                  {step.name}
                </span>
                {index < steps.length - 1 && (
                  <div className={`w-20 h-0.5 mx-4 ${currentStep > step.id ? 'bg-sky-500' : 'bg-gray-200'
                    }`} />
                )}
              </div>
            ))}
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-gray-800 text-2xl">Vehicles</h1>
        </div>
        <Button
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <CreateVehicle
            ...
            onSave={() => {
              ...
              // Here you would typically refresh the vehicle list or add the new vehicle
            }}
          />
        </DialogContent>
      </Dialog>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Vehicle Fleet Overview</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-gray-200 border-b">
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Registration</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Make</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Model</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Cost Centre</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Fleet Number</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Products</th>
                  <th className="px-4 py-4 font-semibold text-sky-600 text-left">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredVehicles.map((vehicle, index) => (
                  ...
                  >
                    <td className="px-4 py-4">
                      <div className="font-medium text-gray-800">{vehicle.registration}</div>
                      <div className="text-gray-500 text-sm">{vehicle.year}</div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="font-medium text-gray-800">{vehicle.make}</div>
                      <div className="text-gray-500 text-sm">{vehicle.fuelType}</div>
                    </td>
                    <td className="px-4 py-4 text-gray-800">{vehicle.model}</td>
                    <td className="px-4 py-4">
                      <div className="max-w-48 text-gray-800 text-sm truncate">
                        {vehicle.costCentre}
                      </div>
                    </td>
                    <td className="px-4 py-4 text-gray-800">{vehicle.fleetNumber}</td>
                    <td className="px-4 py-4">
                      <Badge variant="secondary" className="bg-sky-100 text-sky-700">
                        {vehicle.productCount} items
                      </Badge>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="hover:bg-sky-50 p-0 border-sky-200 w-8 h-8"
                        >
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          className="hover:bg-orange-50 p-0 border-orange-200 w-8 h-8"
                        >
