...
function AccountDetailPageContent() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setRequestCache((prev) =>
          new globalThis.Map(prev).set(requestKey, {
            data: {
  ...
  const handleVehicleFileSelected = async (
    ...
    const costCode = String(
      customer?.new_account_number || accountId || "",
    ).trim();
    if (!costCode) {
      toast.error("Cost code is missing for this client.");
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      setLastVehicleImportSummary(null);

      const payload = new FormData();
      payload.append("file", file);
      payload.append("cost_code", costCode);
      payload.append("company", customer?.company || "");
      ...
      if (!response.ok) {
        throw new Error(
          result?.details || result?.error || "Failed to import vehicles",
        );
      }

      setLastVehicleImportSummary({
        inserted: Number(result?.inserted || 0),
        missingColumns: Array.isArray(result?.missingColumns)
      ...
      toast.success(
        `Imported ${result?.inserted || 0} vehicle rows into vehicles and vehicles_duplicate.`,
      );
      ...
      <div className="space-y-6 p-6">
        <div className="py-12 text-center">
          ...
          <p className="text-gray-500">
            The requested customer account could not be found.
          </p>
        </div>
      </div>
    );
      ...
      case "vehicles":
        return (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <Badge variant="outline">
                  {vehiclesTotalCount} total vehicles
                </Badge>
                <input
            ...
            {lastVehicleImportSummary && (
              <Card>
                <CardContent className="pt-6">
                  <div className="flex flex-col gap-2 text-sm">
                    <div className="font-medium text-green-700">
                      Imported {lastVehicleImportSummary.inserted} vehicle row
                      {lastVehicleImportSummary.inserted === 1
                        ? ""
                        : "s"} from{" "}
                      {lastVehicleImportSummary.sheetName || "the workbook"}.
                    </div>
                    {lastVehicleImportSummary.missingColumns?.length > 0 && (
                      <div className="text-amber-700">
                        Unmatched Excel columns:{" "}
                        {lastVehicleImportSummary.missingColumns.join(", ")}
                      </div>
                    )}
                  </div>
                </CardContent>
                ...
                {vehiclesTotalPages > 1 && (
                  <div className="flex justify-between items-center mt-6">
                    <div className="text-gray-700 text-sm">
                      Showing {(vehiclesPage - 1) * 30 + 1} to{" "}
                      {Math.min(vehiclesPage * 30, vehiclesTotalCount)} of{" "}
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleVehiclesPageChange(vehiclesPage - 1)
                        }
                      ...
                      <div className="flex items-center gap-1">
                        {/* Show page numbers */}
                        {Array.from(
                          { length: Math.min(vehiclesTotalPages, 5) },
                          (_, i) => {
                            const pageNum = i + 1;
                            const shouldShow =
                              pageNum <= 3 ||
                            ...
                            if (!shouldShow) {
                              ...
                              if (
                                pageNum === vehiclesTotalPages - 3 &&
                                vehiclesPage < vehiclesTotalPages - 4
                              ) {
                                return (
                                  <span
                                    key={`ellipsis-${pageNum}`}
                            ...
                            return (
                              <Button
                                key={pageNum}
                                variant={
                                  vehiclesPage === pageNum
                                    ? "default"
                                    : "outline"
                                }
                                size="sm"
                                onClick={() =>
                                  handleVehiclesPageChange(pageNum)
                                }
                      ...
                      <Button
                        ...
                        disabled={
                          vehiclesPage === vehiclesTotalPages || vehiclesLoading
                        }
                      >
                        Next
                      </Button>
