...
const BILLING_LOCK_KEY = "billing";

export const BILLABLE_VEHICLE_FIELDS = [
...
export const BILLABLE_VEHICLE_FIELD_SET = new Set<string>(BILLABLE_VEHICLE_FIELDS);

export const getBillingLock = async (
  supabase: Awaited<ReturnType<typeof createClient>>,
) => {
  const { data, error } = await supabase
    .from("system_locks")
    .select("*")
    .eq("lock_key", BILLING_LOCK_KEY)
    .maybeSingle();
...
export const isBillingLocked = async (
  supabase: Awaited<ReturnType<typeof createClient>>,
) => {
  const lock = await getBillingLock(supabase);
  return Boolean(lock?.is_locked);
};

export const findTouchedBillableVehicleFields = (
  ...
  return Object.keys(payload).filter((key) =>
    BILLABLE_VEHICLE_FIELD_SET.has(key),
  );
};

export const findPresentBillableVehicleFields = (
  payload: Record<string, unknown> | null | undefined,
) => {
  if (!payload || typeof payload !== "object") {
    return [];
  ...
  return Object.entries(payload)
    .filter(([key, value]) => {
      if (!BILLABLE_VEHICLE_FIELD_SET.has(key)) {
        return false;
      }
