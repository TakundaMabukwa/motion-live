...
(async () => {
  const [paymentsRes, allocRes] = await Promise.all([
    supabase.from('account_invoice_payments').select('id,account_number,invoice_number,amount,payment_reference,payment_date,notes'),
    supabase.from('account_payment_allocations').select('payment_id,allocation_type,amount')
  ]);
  ...
  const mismatches = payments.filter(p => Math.abs(round(p.amount) - round(allocByPayment.get(String(p.id||'')) || 0)) > 0.01).map(p => ({ payment_id: p.id, account_number: p.account_number, invoice_number: p.invoice_number, payment_amount: round(p.amount), allocation_total: round(allocByPayment.get(String(p.id||''))||0), payment_reference: p.payment_reference, notes: p.notes }));
  console.log(JSON.stringify({ payments_checked: payments.length, allocation_rows: allocs.length, mismatch_count: mismatches.length, mismatch_sample: mismatches.slice(0,20) }, null, 2));
})();
